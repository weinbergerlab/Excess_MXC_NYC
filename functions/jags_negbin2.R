  ...
  "model
{
  ...
  #2nd likelihood is for training period and does not have AR1
  n.train[t,d] ~dnegbin(p2[t,d],r1)
    # Conversions
  p2[t,d] <- r1/(r1+baseline[t,d])
  ...
  sum.lambda[t] <- sum(lambda[t,1:D])
    ...
    # Conversions
  p3[t] <- r1/(r1+baseline2[t])
  baseline.n[t] ~dnegbin(p3[t],r1)
  
  p4[t] <- r1/(r1+lambda[t, (D+1)])
  sum.n[t]  ~ dnegbin(p4[t],r1)

  excessN[t] <- sum.n[t] - baseline.n[t]
  }

    #Time Series Random Effect--starts D time points from end
